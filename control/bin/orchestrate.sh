...
# Minimal helper to discover repo root
COMMON="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../tools/bash/lib" &>/dev/null && pwd -P)/common.sh"
...
usage() {
  cat <<'USAGE'
Operator entrypoint for common flows (on-prem build, DR, burst).
...
Usage:
  ...
  orchestrate.sh dr <azure|gcp>
      ...
      You may also export CLOUD_PROVIDER=azure|gcp instead of passing the arg.
...
Notes:
  - Thin wrapper over root Makefile targets.
USAGE
}
