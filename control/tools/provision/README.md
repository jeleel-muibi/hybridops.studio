...
- `output/artifacts/packer-builds/init/<timestamp>/` (when `evidence_packer.sh` is available)
...
- Creates a timestamped folder under `output/artifacts/packer-builds/`.
...
- Proof artifacts under `output/artifacts/packer-builds/`
...
grep -R "${CHAIN_ID}" output/artifacts/packer-builds
```
